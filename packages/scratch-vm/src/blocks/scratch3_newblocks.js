...
class Scratch3NewBlocks {
    constructor (runtime) {
        ...
        this.socket.on('joined', function(return_obj) {
          ...
          log.log(return_obj)
          window.open(window.location.href + "?roomid=" + return_obj["room"], '_blank', 'location=yes,height=' + dim["height"] + ',width=' + dim["width"] + ',scrollbars=yes,status=yes');
          isChannelReady = true;
        });
        ...
        this.socket.on('message', function(message) {
          ...
          if (this.clientID != message.clientID) {
            ...
            if (message.message["x"]) {
              ...
              drawACircle(message.message["x"]*document.body.clientWidth, message.message["y"]*document.body.clientWidth);
            }
    ...
    drawCircle(args, util) {
      log.log("drawing circle");
       window.open(window.location.href + "?roomid=" + this.roomID, '_blank', 'location=yes,height=570,width=520,scrollbars=yes,status=yes');
    }
    ...
    fetchData (args) {
      if (this.isRunning) {
        log.log(this.receivedData)
      } else {
       		log.log("nawt connected")
          this.isRunning = true;
      }
    }
    writeRoom (args) {
